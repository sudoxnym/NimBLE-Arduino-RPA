...
/**
 * @brief Resolve a Resolvable Private Address (RPA) using the provided IRK.
 * @param rpa The Resolvable Private Address to resolve.
 * @param irk The Identity Resolving Key (IRK).
 * @return int 1 if resolved successfully, 0 otherwise.
 */
int NimBLEUtils::ble_ll_resolv_rpa(const uint8_t *rpa, const uint8_t *irk) {
    int rc;
...
int NimBLEUtils::ble_ll_resolv_rpa(const uint8_t *rpa, const uint8_t *irk) {
    const uint32_t *irk32 = (const uint32_t *)irk;
    uint32_t ecb_key[4], ecb_pt[4], ecb_ct[4];
    ...
    // Init plain text with zero
    ecb_pt[0] = 0;
    ecb_pt[1] = 0;
    ecb_pt[2] = 0;
    ecb_pt[3] = 0;

    // Set the prand in the plaintext
    ecb_pt[3] = (rpa[3] << 16) | (rpa[4] << 8) | rpa[5];

    // Simulate encryption (replace with real encryption function later)
    ble_hw_encrypt_block(ecb_key, ecb_pt, ecb_ct);
    
    // Compare the hash
    return (ecb_ct[3] == ((rpa[0] << 16) | (rpa[1] << 8) | rpa[2])) ? 1 : 0;
}
